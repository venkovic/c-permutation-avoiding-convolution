...
int main(int argc, char** argv) {
  ...
  fftw_plan p = fftw_plan_dft_2d(n1, n2, fftw_in, fftw_out, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_NO_SIMD);
  ...
  if (do_verify) {
    ...
    // Radix-2
    ifft_2d(re, im, n1, n2, 2, twiddle_re1, twiddle_im1, twiddle_re2, twiddle_im2, rho);
    ...
    if (supports_radix_4(t2_1) && supports_radix_4(t2_2)) {
      ...
      ifft_2d(re, im, n1, n2, 4, twiddle_re1, twiddle_im1, twiddle_re2, twiddle_im2, rho);
      fftw_execute(p);
      for (int i = 0; i < n; i++) {
        fftw_out_flat[2 * i]     /= n;
        fftw_out_flat[2 * i + 1] /= n;
  ...
  if (do_bench) {
    ...
    uifft_2d(re, im, n1, n2, 2, twiddle_re1, twiddle_im1, twiddle_re2, twiddle_im2);
    ...
    if (supports_radix_4(t2_1) && supports_radix_4(t2_2)) {
      ...
      ifft_2d(re, im, n1, n2, 4, twiddle_re1, twiddle_im1, twiddle_re2, twiddle_im2, rho);
      apply_permutation_split(re, im, n, rho);
      uifft_2d(re, im, n1, n2, 4, twiddle_re1, twiddle_im1, twiddle_re2, twiddle_im2);
